...
$(() => {
  $('.expandable-text .btn-expand-toggle').click(event => {
    event.preventDefault();

    const expandableText = $(event.target).parents('.expandable-text');
    expandableText.find('.expanded').toggleClass('d-none')
    expandableText.find('.collapsed').toggleClass('d-none')
  });
});
$(() => {
  // Hides and shows prev/next arrows on book carousels
  $('.books-carousel').each(function() {
    ...
    $carousel.on('slide.bs.carousel', function(event) {
      ...
      } else if (slideIndex === totalItems - 1) {
...
class EmailHint {
  constructor(submit) {
    this.submit = $(submit);
    this.form = this.submit.parents("form");
    this.emailInput = this.form.find("input[name*=email]");
    this.emailHint = this.form.find(".email-hint");
    this.suggestedEmail = this.emailHint.find(".suggested-email");
  }

  mount(onSubmit = () => {}) {
    this.submit.click((event) => {
      event.preventDefault();

      // If the hint is already visible or we can't load Mailcheck, submit
      if (!this.emailHint.hasClass("d-none") || !this.emailInput.mailcheck) {
        this.form[0].requestSubmit(this.submit[0]);
        onSubmit();
        return;
      }

      this.emailInput.mailcheck({
        suggested: (element, suggestion) => {
          this.emailHint.removeClass("d-none");
          this.suggestedEmail.text(suggestion.full);
          this.suggestedEmail.focus();
        },
        empty: () => {
          this.form[0].requestSubmit(this.submit[0]);
          onSubmit();
    ...
    this.suggestedEmail.click(() => {
      this.emailInput.val(this.suggestedEmail.text());
      this.suggestedEmail.text("");
      this.emailHint.addClass("d-none");
    });
  }
};
// ...
...
$(() => {
  ...
  const fitLargeText = () => {
    const textFitElem = $(".text-fit:visible");
    if (textFitElem.length === 0) return;

    let maxSizeMobile = 56;
    let maxSizeDesktop = 72;
    if (textFitElem.hasClass("text-fit-42")) {
      maxSizeMobile = 42;
      maxSizeDesktop = 56;
    }
    if (textFitElem.hasClass("text-fit-90")) {
      maxSizeMobile = 56;
      maxSizeDesktop = 90;
    }
    ...
    textFit(textFitElem, {
      ...
      reProcess: true,
      widthOnly: true,
      maxFontSize: window.matchMedia("(min-width: 768px)").matches ? maxSizeDesktop : maxSizeMobile,
    });
  };
  ...
  $('.newsletter-btn').each((i, submitBtn) => {
    new EmailHint($(submitBtn)).mount(() => $("#newsletter-module")[0]?.scrollIntoView());
  });

  const emailCollectionModal = $(".email-collection-modal");
  if (emailCollectionModal.length > 0) {
    const preview = emailCollectionModal.hasClass("preview");

    const hideCookieSet = () => window.Cookies?.get("_amp_hide_email_collection_pop_up");
    const visibleModal = () => $('.modal.show').is(':visible');
    const visibleFocusedInput = () => $('input,textarea,select:focus').is(':visible');

    setTimeout(() => {
      if (preview || !(hideCookieSet() || visibleModal() || visibleFocusedInput())) {
        emailCollectionModal.modal("show");
      }
    }, emailCollectionModal.data("delay"));

    emailCollectionModal.on("hide.bs.modal", () => {
      if (!preview && !hideCookieSet()) {
        window.Cookies.set("_amp_hide_email_collection_pop_up", "true", { expires: 1 });
      }
    });
  }
});
